...
void InventoryItem::onMouseDragStart(Event::Mouse* event)
{
    ...
    Game::getInstance()->mixer()->playFile(Audio::IMixer::Category::SFX, "sound/sfx/ipickup1.acm");
    _oldType = type();
    setType(Type::DRAG);
...
void InventoryItem::onMouseDragStop(Event::Mouse* event)
{
    ...
    Game::getInstance()->mixer()->playFile(Audio::IMixer::Category::SFX, "sound/sfx/iputdown.acm");
    setOffset({0, 0});
    setType(_oldType);

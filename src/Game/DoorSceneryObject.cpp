        ...
        void DoorSceneryObject::use_p_proc(CritterObject* usedBy)
        {
            ...
            if (!opened()) {
                if (UI::AnimationQueue* queue = dynamic_cast<UI::AnimationQueue*>(this->ui())) {
                    ...
                    if (_soundId) {
                        Game::getInstance()->mixer()->playFile(Audio::IMixer::Category::SFX, std::string("sound/sfx/sodoors") + _soundId + ".acm");
                    }
                }
            } else {
                if (UI::AnimationQueue* queue = dynamic_cast<UI::AnimationQueue*>(this->ui())) {
                    ...
                    if (_soundId) {
                        Game::getInstance()->mixer()->playFile(Audio::IMixer::Category::SFX, std::string("sound/sfx/scdoors") + _soundId + ".acm");
                    }
                }
            }
        }
